...
\item \textbf{Hard gates.} $\xi_x\le1$, $\xi_x^{\mathrm{tr}}\le1$,
	\cref{eq:ampgates}, $d_{\mathrm D}$ within budget for the upper or full
	role, and $f_L>f_{\mathcal B,\mathrm h}$ when $\Le$ is reported.
	The per-driver box cap is enforced when constructing the alignment;
	realized $\Qtc$ above the preference is reported rather than rejected.
	Split systems additionally require an even manifold count and a nominal
	crossover no higher than the declared manifold ceiling. Failure at any
	band frequency rejects the candidate, with the binding gate reported.
