...
Five distinct pressure references are used, and no figure or table mixes
them:
...
\item[B1.] \emph{Intrinsic ceiling}: one driver, half space,
	...
	a system.
\item[B2.] \emph{Listening-area reference demand}: the level required at the
...
\item[B5.] \emph{One full-band stereo channel}: below $f_{\mathrm{sp}}$
	the two channels carry the same mono bass and the architecture study
	credits their equal-pressure sum at B2; above $f_{\mathrm{sp}}$ each
	channel independently meets B4. The primary comparison uses the
	favourable \SI{6}{dB} reference-position credit and repeats the
	canonical case at \SI{3}{dB}.
\end{enumerate}
...
The canonical scenario used for the worked figures and tables is shown
below. These values instantiate the serializable inputs; they are not fixed
constants of the procedure, and driver size and count are outputs.
...
\begin{center}
	...
	\toprule
		...
		manifold integration & maximum nominal crossover
			$\SI{80}{\hertz}$; low-pass attenuation required above\\
		...
		alignment target & preferred $\Qtc^\ast=0.55$; not a hard gate\\
		enclosure limit & $\Vb\le4\Vas$ per physical driver;
			total volume reported as an objective\\
...
\item[A9.] \emph{Summation is restricted to the mono manifold.} The
	...
	velocities are taken as coherent at the listening-area reference: each
...
\item[A12.] \emph{Manifold bandwidth is an external integration
	constraint.} The nominal crossover must not exceed the declared
	manifold ceiling, and the implemented finite-slope low-pass must
	adequately suppress output above the manifold's validated band. The
	Thiele/Small model does not predict manifold cavity modes, unequal path
	lengths, compression or acoustic loading; the ceiling must come from
